...
export default class WamExampleTemplateNode extends WamNode {
  ...
  constructor(module, options) {
    ...
    this._gui = null;
    // Добавляем загрузку нашего сэмпла
    this._loadSample();
  }
  ...
  async _loadSample() {
    const response = await fetch(new URL("./A4.wav", import.meta.url));
    const audioData = await response.arrayBuffer();
    // Декодируем сэмпл в аудио-буфер
    const audioBuffer = await this.context.decodeAudioData(audioData);

    // Передаем данные сэмпла в Процессор (в AudioWorklet)
    this.port.postMessage({
      type: "sampleData",
      buffer: audioBuffer.getChannelData(0), // берем моно-канал
