...
const getWamExampleTemplateSynth = (moduleId) => {
   ...
   * @class
   */
  // ПЕРЕПИСЫВАЕМ WamExampleTemplateSynthPart:
  class WamExampleTemplateSynthPart {
    constructor(samplesPerQuantum, sampleRate) {
      this._sampleRate = sampleRate;
      this._active = false;
      this._buffer = null; // Здесь будет лежать наш A4.wav
      this._ptr = 0; // Текущий индекс чтения
      this._speed = 1.0; // Скорость (питч)
    }

    setSample(buffer, sampleRate) {
      this._buffer = buffer;
    }
    ...
    start(gain, note) {
      ...
      this._ptr = 0;
      this._gain = gain * 0.5;
      // Расчет скорости относительно A4 (69 нота)
      this._speed = 2.0 ** ((note - 69) / 12.0);
    }

    process(startSample, endSample, signal) {
      if (!this._active || !this._buffer) return false;

      let n = startSample;
      while (n < endSample) {
        // Читаем сэмпл с учетом скорости
        const idx = Math.floor(this._ptr);
        if (idx >= this._buffer.length) {
          this._active = false;
          break;
        }

        signal[n] += this._buffer[idx] * this._gain;
        this._ptr += this._speed; // Двигаем указатель со скоростью питча
        n++;
      }
      return this._active;
    }
  ...
  class WamExampleTemplateSynthVoice {
    ...
    // В классе WamExampleTemplateSynthVoice добавим:
    setSample(buffer, sampleRate) {
      this._leftPart.setSample(buffer, sampleRate);
      this._rightPart.setSample(buffer, sampleRate);
    }
  ...
  class WamExampleTemplateSynth {
    ...
    constructor(
      ...
      while (i < this._numVoices) {
        this._voices.push(
          new WamExampleTemplateSynthVoice(samplesPerQuantum, sampleRate, i)
        );
        i++;
      }
    }
    ...
    // В классе WamExampleTemplateSynth добавим метод:
    setSample(buffer, sampleRate) {
      this._voices.forEach((v) => v.setSample(buffer, sampleRate));
    }
